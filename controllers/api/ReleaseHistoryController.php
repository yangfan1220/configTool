...
class ReleaseHistoryController extends Controller
{
    ...
    public function actionGetReleaseHistoryConfig()
    {
        ...
        if ($configType == 1) {
            $data = ConfigDataReleaseHistoryModifyLog::find()->select(['modify_type', 'key', 'old_value', 'new_value'])->where(['app_id' => \Yii::$app->session['app_id'], 'unique_id' => $uniqueId])->asArray()->all();
        } elseif ($configType == 2) {
            $data = ConfigDataReleaseHistoryAllLog::find()->select(['key', 'value'])->where(['app_id' => \Yii::$app->session['app_id'], 'unique_id' => $uniqueId])->asArray()->all();
        } else {
