...
{
    public static function insertConfigDataReleaseHistory($data, $uniqueId, $currentRecordStyle = 0)
    {
    ...
    public static function insertConfigDataReleaseHistoryModifyLog($modifyDataLog, $releaseName, $uniqueId)
    {
    ...
    public static function insertConfigDataReleaseHistoryAllLog($configDataAll, $releaseName, $uniqueId, $sourceType = 'rollback')
    {
        foreach ($configDataAll as $keysInfo) {
            ...
            $model->config_level_log = $sourceType == 'rollback' ? $keysInfo['config_level_log'] : $keysInfo['config_level'];
            $model->comment_log = $sourceType == 'rollback' ? $keysInfo['comment_log'] : $keysInfo['comment'];
            $model->value_type_log = $sourceType == 'rollback' ? $keysInfo['value_type_log'] : $keysInfo['value_type'];
            $model->create_name_log = $sourceType == 'rollback' ? $keysInfo['create_name_log'] : $keysInfo['create_name'];
            $model->modify_name_log = $sourceType == 'rollback' ? $keysInfo['modify_name_log'] : $keysInfo['modify_name'];
            $model->create_time_log = $sourceType == 'rollback' ? $keysInfo['create_time_log'] : $keysInfo['create_time'];
            $model->update_time_log = $sourceType == 'rollback' ? $keysInfo['update_time_log'] : $keysInfo['update_time'];
            $model->save();
            if ($model->hasErrors()) {
                throw new \Exception(current($model->getFirstErrors()));
            }
    ...
    public static function updateProjectInfo($uniqueId)
    {
        $projectInfoObj = ProjectInfo::findOne(['app_id' => \Yii::$app->session['app_id']]);
        $projectInfoObj->current_released_unique_id = $uniqueId;
        $projectInfoObj->save();
    ...
    {
        return \Yii::$app->db2->createCommand('truncate ' . $tableName)->execute();
    }
